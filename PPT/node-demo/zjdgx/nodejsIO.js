 ...
 *  nodejs io test
 *   example: http://localhost:8080/file_async
 *   		http://localhost:8080/file_sync
 *   		http://localhost:8080/url_async slow
 *   		http://localhost:8080/url_sync
 */
...
http.createServer(function (req, res) {
	...
	getFileData(pn, function (data) {
		if (pn.indexOf("url") > -1) {
			...
			+ "\n\ncharacter number: " + data, 'utf8');
		}
	});
}).listen(8080, function(){
	console.log("nodejsIO start....");
});
...
//----------------------------------------------------------------------
// 5. truncate
fs.truncate('./test/testfile-truncate.txt', 1000, function (err) {
	if (err) throw err;
	console.log('fd');
});
