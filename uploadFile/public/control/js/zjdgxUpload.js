(function($,undefined){
	$.fn.zjdgxUpload = function(options,param){
		...
		if (typeof options == 'string') {
			...
			if($.isFunction(fn)){
				...
				throw ("zjdgxUpload - No such method: " + options);
			}
		}
		...
		return this.each(function(){
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						self.funSetStatusInfo(zjdgxFile.funReturnNeedFiles());  // 显示统计信息
						var html = '', i = 0;
						// 组织预览html
						...
						var funBindDelEvent = function(){
							if($(".file_del").length>0){
								...
								$(".file_del").click(function() {
									zjdgxFile.funDeleteFile(parseInt($(this).attr("data-index")), true);
									return false;	
								});
							}
							...
							if($(".file_edit").length>0){
								...
								$(".file_edit").click(function() {
									// 调用编辑操作
									//zjdgxFile.funEditFile(parseInt($(this).attr("data-index")), true);
									return false;	
								});
							}
						};
					...
					onSuccess: function(file, response) {
						...
						if(para.finishDel){
							...
							$("#uploadList_" + file.index).fadeOut();
							// 重新设置统计栏信息
							self.funSetStatusInfo(zjdgxFile.funReturnNeedFiles());
						}
					},
				...
				zjdgxFile = $.extend(zjdgxFile, params);
				zjdgxFile.init();
			};
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					// 判断当前是否有文件需要上传
					if(zjdgxFile.funReturnNeedFiles().length > 0){
						$("#fileSubmit").click();
					}else{
						alert("请先选中文件再点击上传");
					}
